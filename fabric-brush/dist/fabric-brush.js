(function e(t, n, r) {
  ...
  1: [function(require, module, exports) {
    ...
    (function(fabric) {
      ...
      fabric.CrayonBrush = fabric.util.createClass(fabric.BaseBrush, {
        ...
        update: function(p) {
          this.set(p);
          this._latestStrokeLength = this._point.subtract(this._latest)
            .distanceFrom({
              x: 0,
              y: 0
            });
        ...
        draw: function(ctx) {
          var i, j, p, r, c, x, y, w, h, v, s, stepNum, dotSize,
            dotNum, range;

          v = this._point.subtract(this._latest);
          s = Math.ceil(this._size / 2);
          stepNum = Math.floor(v.distanceFrom({
            x: 0,
            y: 0
          }) / s) + 1;
          v.normalize(s);

          dotSize = this._sep * fabric.util.clamp(this._inkAmount /
            this._latestStrokeLength * 3, 1, 0.5);
    ...
    (function(fabric) {
      ...
      fabric.Drip = fabric.util.createClass(fabric.Object, {
        ...
        initialize: function(ctx, pointer, amount, color, _strokeId) {
          ...
          this.amount = fabric.util.getRandom(amount, amount *
            0.5);
          this.color = color;
        ...
        _update: function(brush) {
          ...
          this._point.addEquals({
            ...
            y: fabric.util.getRandom(this.life * this.amount /
              30)
          });
        ...
        _draw: function() {
          this.ctx.save();
          this.line(this.ctx, this._lastPoint, this._point, this.color,
    ...
    (function(fabric) {
      ...
      fabric.InkBrush = fabric.util.createClass(fabric.BaseBrush, {
        ...
        _render: function(pointer) {
          var subtractPoint, distance, point, i, len, strokes,
            stroke;
          this._strokeCount++;
          if (this._strokeCount % 120 === 0 && this._dripCount <
            10) {
            this._dripCount++;
          }
        ...
        drawSplash: function(pointer, maxSize) {
          ...
          for (i = 0; i < num; i++) {
            ...
            c = fabric.util.getRandom(Math.PI * 2);
            point = new fabric.Point(pointer.x + r * Math.sin(c),
              pointer.y + r * Math.cos(c));

            ctx.fillStyle = color;
            ctx.beginPath();
            ctx.arc(point.x, point.y, fabric.util.getRandom(
              maxSize) / 2, 0, Math.PI * 2, false);
            ctx.fill();
          }
          ctx.restore();
        },
        ...
        _resetTip: function(pointer) {
          ...
          for (i = 0, len = this._strokeNum; i < len; i++) {
            strokes[i] = new fabric.Stroke(this.canvas.contextTop,
              point, this._range, this.color, this.width, this._inkAmount
            );
          }
        }
      });
    ...
    (function(fabric) {
      ...
      fabric.MarkerBrush = fabric.util.createClass(fabric.BaseBrush, {
        ...
        _render: function(pointer) {
          ...
          for (i = 0, len = (this._size / this._lineWidth) / 2; i <
            len; i++) {
            lineWidthDiff = (this._lineWidth - 1) * i;

            ctx.globalAlpha = 0.8 * this.opacity;
            ctx.moveTo(this._lastPoint.x + lineWidthDiff, this._lastPoint
              .y + lineWidthDiff);
            ctx.lineTo(pointer.x + lineWidthDiff, pointer.y +
              lineWidthDiff);
            ctx.stroke();
          }
  ...
  5: [function(require, module, exports) {
    /**
     * SprayerBrush class
     * @class fabric.SprayerBrush
     * @extends fabric.BaseBrush
     */
    (function(fabric) {

      fabric.SprayerBrush = fabric.util.createClass(fabric.BaseBrush, {
        ...
        initialize: function(canvas, opt) {
          ...
          fabric.Image.fromURL(this.sprayerBrushDataUrl, function(
            brush) {
            context.brush = brush;
        ...
        changeColor: function(color) {
          this.color = color;
          this.brush.filters[0] = new fabric.Image.filters.Tint({
            color: color
          });
          this.brush.applyFilters(this.canvas.renderAll.bind(this
            .canvas));
        },
        ...
        _render: function() {
          ...
          function draw() {
            ...
            point = new fabric.Point(context._point.x || 0,
              context._point.y || 0);
            distance = point.distanceFrom(context._lastPoint);
            angle = point.angleBetween(context._lastPoint);
            amount = (100 / context.size) / (Math.pow(distance, 2) +
              1);

            context._inkAmount += amount;
            context._inkAmount = Math.max(context._inkAmount -
              distance / 10, 0);
            ...
            x = context._lastPoint.x + Math.sin(angle) - context.size /
              2;
            y = context._lastPoint.y + Math.cos(angle) - context.size /
              2;
            context.canvas.contextTop.drawImage(context.brush._element,
              x, y, context.size, context.size);
  ...
  6: [function(require, module, exports) {
    ...
    (function(fabric) {
      ...
      fabric.Stroke = fabric.util.createClass(fabric.Object, {
        ...
        initialize: function(ctx, pointer, range, color, lineWidth,
          inkAmount) {
          ...
          this.ctx = ctx;
          this.color = color;
          this._point = new fabric.Point(pointer.x + x0 * cos -
            y0 * sin, pointer.y + x0 * sin + y0 * cos);
        ...
        draw: function() {
          var ctx = this.ctx;
          ctx.save();
          this.line(ctx, this._lastPoint, this._point, this.color,
